...
updated: 2023-12-09 14:30:55 +0900
tags:
  - 컴퓨터-네트워킹-하향식-접근
---
...
**초기 상태 라인**, 6개의 **헤더 라인**, **체 몸체**로 이루어졌다.  
개체 몸체는 요청 객체를 포함한다.  
...
SMTP 는 메시지를 송신 메일 서버에서 수신 메일 서버로 어떻게 전송할까? 다음과 같이 전송한다. 
...
먼저 클라이언트-서버 구조에 대한 분배 시간 ($D_{cs}$) 을 계산할 때 어떤 피어도 파이리을 분배하는 데 도움을 줄 수 없다.
...
새로운 피어 A가 토렌트에 가입할 때 트래커는 참여하고 있는 피어 집합에서 임의로 피어들의 부분집합을 선택하여 해당 피어들의 IP 주소를 A에게 보낸다. A는 이 목록에 있는 모든 피어와 동시에 TCP 연결을 설정하고, 이 피어들을 ‘이웃 피어’ 라고 부르자.  
